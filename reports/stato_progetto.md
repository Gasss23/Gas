...
> Ultimo aggiornamento: **2026-08-19** (R2 implementata — commit atomico `memoria_revisore.md` con `git commit -o`. Review #85 APPROVATO CON RISERVE. Suite hook: **19 PASS**. Branch `fix/r2-durabilita-memoria`.)
> Storico sessioni, dettaglio componenti, finding chiusi: `reports/stato_storico.md`

## Stato motore

FASE 1 ✅, FASE 2 ✅ e **FASE 2.5** ✅ chiuse. **85 review** completate (ultime: #85 = R2 durabilità memoria APPROVATO CON RISERVE; #84 = quotePath non-ASCII APPROVATO CON RISERVE; #83 = T-gate-E APPROVATO; #82 = session-end design fix APPROVATO CON RISERVE; #81 = R-voice-3 APPROVATO). Suite WSL locale (2026-07-27, feature/crm-dup-telefono): **276 PASS, 0 FAIL, 0 SKIP** (dopo `pip install -r requirements.txt -r requirements-dev.txt` sul venv WSL; Python 3.12.3). Hook suite: **14 PASS** (T-hook-b/d/f aggiornati per nuovo contratto session_end). Voice suite: **19 PASS** (+1 R-voice-3). ⚠️ **ERRORE DICHIARATO**: la riga "Suite WSL locale (2026-07-19): 247 PASS, 0 FAIL, 2 SKIP" era FALSA — al 2026-07-19 il venv WSL conteneva SOLO pytest e la suite kernel NON era eseguibile su WSL (dipendenze motore assenti; vedi §7). Il falso accertato è che NON venivano da WSL; l'origine di quei numeri è NON VERIFICATA (ipotesi CI/Codespace, mai confermata da un artefatto). Corretta con dati reali di oggi.
...
**✅ Fetta A — T-gate-E cd fail-closed (2026-08-19, review #83 APPROVATO, branch `fix/nonascii-cd-tests`)**: aggiunto `test_gate_e_cd_fails_blocks` in `TestReviewGateFailClosed` (`tests/test_unit_hooks.py`). Copre il guard `cd "$CLAUDE_PROJECT_DIR" || exit 2` in `review_gate.sh` righe 38-41. T-gate-D copriva 'cd ok, git diff fallisce'; T-gate-E copre 'cd stesso fallisce' (path inesistente). Suite hook: **5 PASS su 5**.
**✅ Fetta B — core.quotePath=false non-ASCII (2026-08-19, review #84 APPROVATO CON RISERVE, branch `fix/nonascii-cd-tests`)**: `check_handoff.py:_diff_names` e `check_verdetto.py:_session_files` ora usano `git -c core.quotePath=false diff --name-only`. Bug: con default `core.quotePath=true` i path non-ASCII arrivavano escapati `"r\303\251..."` e il set comparison produceva falso mismatch. Test `test_nonascii_filename_check_handoff` (falliva prima, passa ora). Riserva #84 aperta: test speculare `test_nonascii_filename_check_verdetto` per `check_verdetto.py` da aggiungere.
**✅ R2 — durabilità memoria revisore su interruzione (2026-08-19, review #85 APPROVATO CON RISERVE, branch `fix/r2-durabilita-memoria`)**: `scripts/commit_memoria_revisore.sh` — usa `git commit -o .claude/agents/memoria_revisore.md` per committare SOLO la memoria anche se altri file sono in staging (motore sotto review). FAIL-SAFE §9: qualsiasi errore → warning in `gas_debug.log` → exit 0. `.claude/agents/revisore.md` aggiornato: istruzione di chiamare lo script dopo ogni riga contatore. Suite hook: **19 PASS** (+4 T-R2-a/b/c/d). `.gas_history.json` ESCLUSO (finding aperto separato). Idempotente con /fine-task passo 4 (git add è no-op se già committato).
...
- **Endpoint HTTP voice** (FASE 3 Fetta 1, review #76+#77): `modules/voice/server.py` — `POST /voice`, bearer auth, kernel singleton, single-thread stdlib, fail-closed su token assente
...
- 🟡 **#84-riserva check_verdetto non-ASCII**: `check_verdetto.py:_session_files` ha ricevuto il fix `core.quotePath=false` ma nessun test speculare. Aggiungere `test_nonascii_filename_check_verdetto` in una fetta successiva (analogo a `test_nonascii_filename_check_handoff`).
- ✅ **R2 — durabilità memoria su interruzione (2026-08-19, CHIUSA)**: `scripts/commit_memoria_revisore.sh` implementato. Dogfooding verificato dal revisore: HEAD=SOLO memoria, staging intatto, exit 0. Finding residuo: `.gas_history.json` durabilità runtime (VPS/snapshot) — finding aperto separato, NON chiuso da R2.
...
- ✅ **Bonifica branch remoti ESEGUITA** (2026-07-22, aggiornata 2026-07-28): da **27 head a 4** (main + 3 non mergiati). I 22 branch mergiati in `main` sono stati cancellati da origin. In seguito **`feature/crm-dup-detect` cancellato da origin** (superato dal contenuto: fetta 3 riscritta da zero in PR #47 → `feature/crm-dup-telefono` → merge). Rimangono **3 NON mergiati**, tutti da NON cancellare senza verifica: `fix/crm-idemp-diario`, `fix/review44-riserve-AC`, `claude/phone-gas-development-10svqc`. Totale head corrente = **4** (main + 3). ℹ️ **Nota storica**: `feature/crm-dup-detect` (branch `1d32819`, review #49) era in questa lista; il suo contenuto è stato superato dalla rewrite PR #47 (#67) ed è stato poi eliminato da origin — vedi lezione push --delete qui sotto. Metodo di misura: `git ls-remote --heads origin`. Azione: **nessuna traccia in git**, registrata qui. ⛔ CANCELLAZIONE = azione umana, MAI da sessione agente.
