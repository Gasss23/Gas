...
> Ultimo aggiornamento: **2026-08-29** (sonda/vps-stato-2026-08-26: Fetta A prompt hardening + Fetta B tool calcola() — review #93 APPROVATO CON RISERVE. Suite: 292 PASS. Report: `reports/ultimo_report.md`.)
> Storico sessioni, dettaglio componenti, finding chiusi: `reports/stato_storico.md`

## Stato motore

FASE 1 ✅, FASE 2 ✅ e **FASE 2.5** ✅ chiuse. **93 review** completate (ultime: #93 = prompt hardening + calcola() APPROVATO CON RISERVE; #92 = R-phantom-pr-1 APPROVATO CON RISERVE; #91 = Fetta 4a client vocale APPROVATO CON RISERVE; #90 = BOCCIATO; #89 = TTS ElevenLabs APPROVATO CON RISERVE). Suite WSL locale (2026-08-29): **292 PASS, 0 FAIL** (+16 T62 nuovi tool calcola). Hook suite: **14 PASS**. Voice suite: **19 PASS**. ⚠️ **ERRORE DICHIARATO**: la riga "Suite WSL locale (2026-07-19): 247 PASS, 0 FAIL, 2 SKIP" era FALSA — al 2026-07-19 il venv WSL conteneva SOLO pytest e la suite kernel NON era eseguibile su WSL (dipendenze motore assenti; vedi §7). Il falso accertato è che NON venivano da WSL; l'origine di quei numeri è NON VERIFICATA (ipotesi CI/Codespace, mai confermata da un artefatto). Corretta con dati reali di oggi.
...
- 🟡 **kernel rifiuta 7×8 — DIAGNOSTICATO (2026-08-29, sonda read-only)**: causa radice confermata verbatim. Il system prompt (`gas.py:50-55`) forza `run_command` per TUTTI i calcoli + "dichiara incertezza se non puoi verificare". La `SHELL_ALLOWLIST` (`gas.py:874-878`) non include nessun tool aritmetico (`bc`, `python`, `expr`). Il modello Groq (primo rung attivo su questo host) segue le istruzioni: nessuna tool call tentata, `final` emesso con testo "Non ho a disposizione...". Pipeline e provider funzionano correttamente — il bug è semantico. Tre opzioni di fix proposte in `reports/ultimo_report.md` §4 (A: eccezione nel system prompt; B: aggiunta `bc` all'allowlist; C: tool `calcola` dedicato). **Decisione e fetta di fix delegate all'operatore.**
