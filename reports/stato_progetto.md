...
> Ultimo aggiornamento: **2026-08-21** (sonda/voice-client-env: sonda ambiente client vocale FASE 3 — audio WSLg, librerie, D1-ter IP. Nessun codice scritto. Esiti: play WSL→Windows via ffmpeg+WSLg ✅, solo ffmpeg disponibile, localhost:8765 stabile via WSL2 forwarding. Design fette 4a/4b proposto in reports/ultimo_report.md.)
...
- **Endpoint HTTP voice** (FASE 3 Fetta 1+2, review #76+#77+#88): `modules/voice/server.py` — `POST /voice`, bearer auth, kernel singleton, single-thread stdlib, fail-closed su token assente; `modules/voice/stt.py` — STT Groq Whisper, routing audio/JSON, tutti i rami d'errore, zero nuove dipendenze
