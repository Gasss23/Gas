...
**✅ FASE 3 Fetta 4b — client browser HTML5 (2026-09-15, branch `feat/voice-client-4b`)**: `clients/voice/browser_server.py` (proxy HTTP stdlib, 127.0.0.1:9000 → :8765) + `clients/voice/browser_client.html` (getUserMedia → POST /voice → play MP3). Approccio same-origin via proxy: zero CORS, zero modifiche a modules/. Token GAS_VOICE_TOKEN ruotato (vecchio esposto). Test E2E reale programmático: GET / → HTML 200/31ms; POST /voice JSON → 200 1.31s; POST /voice WAV → MP3 200 4641B 1.87s. Require: voice server avviato su :8765 + browser apre http://127.0.0.1:9000. Nessuna nuova dipendenza. gate di review NON richiesto (diff non tocca gas.py/brains/modules/tests/).
...
- ✅ **FIX promemoria_end.sh — blocco JSON Stop hook (2026-09-23, branch sonda/fine-task-recon-2026-09-23, PR #95, review #101 APPROVATO)**: `promemoria_end.sh` riscritto da soft-warning stderr a blocco JSON `{"decision":"block","reason":"..."}` su stdout. Logica "handoff fresco" = ultimo commit non-`chore(scrivi-rep):` in `BASE..HEAD` tocca `reports/handoff.md`. Filtro chore da SESSION_COMMITS. Anti-loop su `stop_hook_active`. Fail-open totale. 9 test su repo git reali (T-prom-1..7+3b+3c), 34/34 green. Test e2e manuale: blocco JSON confermato. F3 (partenza su main) fuori scope, in DECISIONI UMANE.
- ✅ **Autonomia #1 — K0+K1+K2 (2026-09-22, branch sonda/autonomia-studia-cap1)**: prima fetta di codice. Decisione architetturale capo-eng: knowledge in `.gas_knowledge.db` SEPARATO dal diario. File creati: `knowledge/sources.yaml` (catalogo fonti), `knowledge/test_source.txt` (fonte test), `tools/ingest_knowledge.py` (CLI off-loop). Test: 2 chunk ingeriti (1505+314 chars), secondo run idempotente (skipped=2, ingested=0), `.gas_memory.db` mtime invariato. Nessuna modifica a gas.py/run_turn/loop. K3 (wiring `ricorda`) e K4 (protezioni) rimandati.
...
- ✅ **R-tts-1 CHIUSA** (2026-09-02, review #96 APPROVATO CON RISERVE, branch `fix/tts-cap-testo`, PR #82): `_cap_text()` in `tts.py` — cap deterministico `GAS_TTS_MAX_CHARS` (default 2000) con troncamento a confine frase/spazio/hard-cut. WARNING loggato. 7 test TVT-cap-* (24/24 PASS). Riserve del revisore risolte nella stessa sessione: R-tts-cap-1 (try/except ValueError su env var) e R-tts-cap-2 (assert sostituito con if-cut).
...
- ✅ **F1 CRITICO — kernel rifiuta 7×8 — CHIUSO (2026-09-01, review #95 APPROVATO, branch `fix/chiusura-f1-calcola-2026-09-01`)**: causa radice identificata (audit 2026-08-29) e fix già applicato nel commit `62af5ee` (review #93 — "prompt hardening Fetta A"). Il system prompt a `gas.py:49-50` ordina ora esplicitamente `calcola()` per calcoli aritmetici; `run_command` delimitato a "CONTEGGI E MISURE SU FILE" (riga 51). `SHELL_ALLOWLIST` verificata: nessun calcolatore presente (bc/python/expr/awk assenti, riga 992-996). Sonda E2E 2026-09-01 su ENTRAMBI i provider con history temporanea isolata: Gemini (gemini-flash, flash-lite a 429) → `calcola("7*8")→56`, `calcola("math.sqrt(144)")→12.0`; Groq (solo GROQ_API_KEY) → idem. 4 PASS su 4. Contraddizione CAVEAT (ii) riconciliata: audit 2026-08-29 usava il vecchio prompt (pre-fix), PR #78 e questa sonda usano il prompt corretto (post-fix) — Groq consistente con new prompt. Suite: 299 PASS, 0 FAIL. Revisore: APPROVATO #95.
...
5. ✅ **GAS risponde SEMPRE in italiano** — **COMPLETATO** (2026-09-21, PR feat/lang-rule-italian). Regola di lingua rafforzata in `_GAS_SYSTEM_PROMPT_BASE` (gas.py:48) e aggiunta in cima a `gas_identity.md`. Test T63a/b/c/d. Revisore #100: APPROVATO. **Sonda E2E reale (2026-09-21)**: input inglese "Hello, what is 7 times 8 and who are you?" → Groq (gpt-oss-120b) risponde in italiano dal primo messaggio: "Il risultato è **56**. Io sono **Gas**…" (Gemini a quota oggi, paracadute Groq attivo). CONFERMATO.
