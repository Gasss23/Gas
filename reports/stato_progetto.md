...
- ✅ **R-hook-jq CHIUSO** (2026-07-19, fix/hook-jq-failloud commit `0ced9e0`, review #56 APPROVATO — ⚠️ **gate mitigato**: revisore ha restituito solo la riga di memoria (`#56 — APPROVATO — …`), nessun testo di analisi del diff prodotto; riga verbatim in `ultimo_report.md` copre la tracciabilità per procura; flag #1 assente): `scrivi_rep.sh` ora fail-loud quando jq è assente (trigger rilevato via grep, check funzionale `jq --version`, warning su stderr + exit 0 + nessun file/commit). Commento riga 3 corretto (stantio da main-lock). T-hook-i (jq assente) e T-hook-j (detached-HEAD) aggiunti. 10/10 PASS. Nota tecnica: usato `jq --version` anziché `command -v jq` perché `/bin` → `/usr/bin` (symlink) rende impossibile separare jq da bash/grep nel PATH nei test.
