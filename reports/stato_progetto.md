...
- 🟡 **kernel rifiuta 7×8 — VERIFICATO RISOLTO SU GEMINI (rung 1, gemini-2.5-flash-lite) — sonda E2E 2026-09-01, tool call reale da `.gas_history.json`**: `calcola(expr="7*8")` → 56, `calcola(expr="math.sqrt(144)")` → 12.0. 2 PASS su 2. Rif: `reports/ultimo_report.md` (2026-09-01). Stato: **non chiuso** — tre caveat aperti:
  - ⚠️ **CAVEAT (i) — causa radice NON rimossa**: `gas.py:46-48` ordina `run_command` per tutti i calcoli; `SHELL_ALLOWLIST` non contiene calcolatori (`bc`/`expr`/`awk`/`python` assenti) — audit 2026-08-29, finding F1 CRITICO. Il fix strutturale è **ancora aperto**. Gemini aggira di fatto l'ordine impossibile emettendo `calcola()` invece di `run_command`; il bug SEMANTICO esiste nel motore indipendentemente dall'esito osservato su Gemini.
  - ⚠️ **CAVEAT (ii) — comportamento Groq CONTRADDITTORIO**: diagnosi originale 2026-08-29 riportava "Groq rifiuta 7×8"; sonda E2E PR #78 (`sonda/e2e-calcola-groq-2026-08-29`) riportava Groq 2 PASS. I due esiti sono incompatibili e non riconciliati: devono essere riesaminati prima di qualsiasi dichiarazione definitiva su Groq.
  - ⚠️ **CAVEAT (iii) — attribuzione "Groq-specifico" NON provata**: Groq non è stato testato in questa run (2026-09-01); la dicitura "Groq-specifico" era un'ipotesi formulata PRIMA dei test, non un fatto verificato empiricamente in questa sessione.
  - **Diagnosi storica (mantenuta)**: causa = system prompt `gas.py:46-48` ordina `run_command` per tutti i calcoli; `SHELL_ALLOWLIST` senza calcolatori → ordine impossibile → bug SEMANTICO agnostico dal provider. Gemini lo aggira; comportamento degli altri rung (Groq, OpenRouter, Ollama) non verificato in questa sonda.
