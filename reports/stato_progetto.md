...
- ✅ **R-crm-1b CHIUSO** (2026-07-27, feature/crm-dup-telefono, revisore #67 APPROVATO CON RISERVE). Fetta email ✅ + merge umano ✅ + idempotenza diario ✅ + **telefono (fetta 3) ✅**: `normalizza_telefono` + `rileva_duplicati_telefono` in `modules/memory/store.py`; 22 test T60a–T60m; 272 PASS 0 FAIL. Riserve R1/R2 (non bloccanti): R1 — `int(r["id"])` fuori try/except post-lettura (SQLite garantisce INTEGER PK, non critico). R2 — ramo `chiave_norm` non coperto da T60 (solo `contatto` testato in coppia mista); copertura futura.
