...
> Ultimo aggiornamento: **2026-08-19** (Sonda R2 — durabilità memoria su interruzione: sonda + proposta design completate. Vaglio revisore: proposta strutturalmente solida, 3 precisioni descrittive raccomandate prima dell'implementazione. Nessun codice scritto.)
...
**🟡 Sonda R2 — durabilità memoria su interruzione (2026-08-19, branch `fix/r2-durabilita-memoria`)**: sonda su `memoria_revisore.md` e `.gas_history.json` completata. Proposta di design "commit atomico al momento del verdetto" scritta e vagliate dal revisore (proposta strutturalmente solida). Tre precisioni descrittive raccomandate prima dell'implementazione (push timing, copertura snapshot, caso HEAD detached). `.gas_history.json` dichiarato escluso dal meccanismo — file runtime, già mitigato da snapshot GAS. Nessun codice scritto. **Decisione umana richiesta**: approvare / modificare / rigettare la proposta prima dell'implementazione.
...
- 🟡 **R2 — durabilità memoria su interruzione (2026-08-19)**: proposta design completata, in attesa di approvazione operatore. Meccanismo: commit atomico di `memoria_revisore.md` al momento del verdetto (subagent revisore, dopo Edit/Write riga contatore). `.gas_history.json` escluso (file runtime, snapshot GAS già attivo ma copertura parziale — solo turni con tool call distruttive). Tre precisioni da integrare nel documento prima dell'implementazione: push timing, copertura snapshot non-totale, caso HEAD detached. Fonte: `reports/ultimo_report.md` §3 + §4.
